...
#eigentlich
y=((Z_out*4*pi*(0.0039+0.0017)**2)/(Z_Quelle*F))*((4*pi*(0.0039+0.0017+0.0041)**2)/(F*np.cos(theta*2*pi/(2*360))))*((1)/(N_Folie))
#print(y)
...
Z=79
E_a=5486
y_2=(1/(4*pi*e_0)**2)*((z*Z*con.e)/(4*5486*10**6))**2 * (1/( np.sin(((x-2)*2*pi/360)*pi/360)**4) ) 
#bei python ist entspricht np.sin(x*2*np.pi/360) sin(x) 6m taschenrechner
#print(y)

plt.plot(theta,y,"rx", label="Messwerte", mew=2, ms=6)
plt.plot(x,y_2, "black", label="Theoriekurve")
plt.xlabel("$\Theta$/Grad")
plt.ylabel("$d\sigma / d\Omega$")
plt.ylim(0, 4*10**(-21))
plt.legend()
#plt.show()
plt.savefig("ruther.png")
